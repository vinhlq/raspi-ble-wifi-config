...
def find_adapter_path_in_objects(dbus_interface, pattern=None):
    objects = get_managed_objects()
    for path, ifaces in objects.items():
        adapter = ifaces.get(dbus_interface)
        if adapter is None:
            continue
...
def get_adapter_address(adapter_path):
    objects = get_managed_objects()
    for path, ifaces in objects.items():
...
def find_gatt_object(pattern=None):
    """
    Returns the first object that the bluez service has that has a GattManager1 interface
    """
    bus = dbus.SystemBus()
    path = find_adapter_path_in_objects(GATT_MANAGER_IFACE, pattern)
    return bus.get_object(BLUEZ_SERVICE_NAME, path)

def find_adapter_object(pattern=None):
    """
    Returns the first object that the bluez service has that has a GattManager1 interface
    """
    bus = dbus.SystemBus()
    path = find_adapter_path_in_objects(ADAPTER_INTERFACE, pattern)
    return bus.get_object(BLUEZ_SERVICE_NAME, path)

def remove_all_devices(adapter_pattern=None):
    adapter_object = find_adapter_object(adapter_pattern)
    objects = get_managed_objects()
    for path, ifaces in objects.items():
        dev = ifaces.get(DEVICE_INTERFACE)
        if dev is None:
            continue
        if (path.startswith(adapter_object.object_path)):
            print("Remove device: path: {path}, address: {address}".format(path=path, address=dev["Address"]))
            adapter_manager = dbus.Interface(adapter_object, ADAPTER_INTERFACE)
            adapter_manager.RemoveDevice(path)
...
class Characteristic(dbus.service.Object):
    ...
    def PropertiesChanged(self, interface, changed, invalidated):
        logger.info("Default PropertiesChanged called, returning error")
        pass
